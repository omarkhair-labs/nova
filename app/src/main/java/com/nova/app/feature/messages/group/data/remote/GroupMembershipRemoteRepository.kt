...
class GroupMembershipRemoteRepository(
    context: Context,
    ...
    override suspend fun deleteGroup(conversationId: Long): ApiResult<Unit> {
        return authenticatedCall { token ->
            when (
                val response = requestJson(
                    path = "conversations/$conversationId/group/",
    ...
    private fun parseDetail(json: JSONObject): GroupDetail {
