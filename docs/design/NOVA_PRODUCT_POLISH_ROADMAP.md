...
The 2026-08-28 Samsung closure investigation proved a second, shared client root cause rather than four backend validation defects. Edit Profile, Post, Story and Pulse all used `NovaApiClient.requestMultipart()` with `HttpURLConnection.setChunkedStreamingMode`, which emitted `Transfer-Encoding: chunked` without `Content-Length`; through the deployed Android -> Traefik -> Daphne/ASGI path the multipart content type arrived but DRF received no fields or files. The exact backend response bodies and byte lengths matched empty-request validation. The shared transport now uses an OkHttp `MultipartBody` with known content length while streaming file-backed media, and focused transport tests cover UTF-8 text-only multipart, Profile `PUT` without an avatar, a Post image, and Story/Pulse video plus thumbnail. Safe diagnostics record only method/path, field names, file-field names, content length, status and parsed failure class/message.

Pulse root media publishing now uses the same account-scoped durable WorkManager pipeline as Post/Reel/Story, including persistable picker access, H.264/AAC preparation and duration integrity, first-frame thumbnail, category preservation, progress, bounded automatic retry, manual retry/cancel state and server-side per-account publish idempotency. The selected-video preview is audible by default with an accessible 48dp mute control, and the shared player reacts to mute/playback changes so a Pulse viewer yields while its reply composer is open. Migration `0036_pulse_publish_identity` must be applied with the backend deployment. This closure is `PR READY`, not device verified.
...
The next Samsung pass must also re-run the formerly empty multipart paths on the deployed build: save Profile theme/metadata without selecting a new avatar, publish one image Post, publish one image/video Story, and publish one image/video Pulse. For Pulse, verify audible selected-video preview and mute toggling, status continuity after closing the composer, network-loss retry/cancel behavior, successful feed reconciliation without duplication, first-frame visibility, picture plus audio for full source duration, and full-screen/background return. No result is claimed until the corrected Android build and migration are deployed together.
